...
const NOTES=[
 {v:"1.0.10",t:"Body-part aiming rebuilt · no more overshooting",items:[
   ["Body-part aiming was on a completely separate path","It grabbed whichever colour blob was biggest each frame, kept no lock, and never told the rest of the system when it changed its mind — so with two people on screen the aim jumped to whoever was momentarily larger, and that jump was treated as the target sprinting across the screen. Peak pointer speed with two figures: 4583px/s, against 247px/s for the same scene in normal colour mode. It now picks and locks a target exactly the way colour mode does and only then splits it into body parts, so it finally inherits the lock, the aim lock-in and the follow speed. Same scene now: 229px/s."],
   ["A person is rarely one blob","Different colours for hair and shirt, a dark strap, part of the body behind cover — all of it arrives as separate pieces, and aiming at the biggest piece means aiming at whichever one wins that frame. Nearby pieces are now assembled into one figure — all still only the colours you picked — with a distance limit so somebody standing nearby doesn't get absorbed into them."],
   ["Regions only use that figure's own pixels","The head/torso/leg bands used to sweep up every colour blob that overlapped them, so a second person standing in the band dragged the aim toward themselves — it would sit between two people while telling you it was on a head."],
   ["It can't be pushed past what it's aiming at","Two ways this happened. The catch-up push and the keep-pace push add together and could overshoot, which puts the error on the other side so the next moment pushes back — that's an oscillation, and at low smoothness there's almost nothing damping it. And worse, the keep-pace push could deadlock against the catch-up pull and park the pointer a fixed distance in front — 129px ahead of a stationary target in the worst measured case. Both are now impossible by construction."],
   ["Aim lock-in works while moving too","It used to switch off entirely once a target was moving, which assumes camera noise stops when things move. It doesn't. Movement happens along one direction while noise is scattered everywhere, so it now follows the along-direction instantly and only steadies the sideways part."],
 ]},
