...
[![Version](https://img.shields.io/badge/version-1.0.6-a855f7)](https://github.com/slayfoids/curse-assist/releases)
...
**Download [`CursorAssist-v1.0.6.exe`](https://github.com/slayfoids/curse-assist/releases)
...
| **Body-part detection** | Off (default) = guide to the color directly. On = aim at a body region of the **locked** figure. It runs the same target selection as colour mode and only then splits that target into regions, so it inherits the lock, switch detection, aim lock-in and follow speed. The figure is assembled from its own nearby pieces — a person is rarely one blob once hair, shirt, straps and cover are involved — with a distance limit so a bystander is not absorbed, and only that figure's own pixels count toward its region bands. |
